...
class AdminController extends Controller
{
	...
	public function addAction()
	{
		if(!empty($_POST))
		{
			...
			if(!$id)
			{
				$this->view->message('success', 'error accept');
			}
			$this->model->postUploadImage($_FILES['img']['tmp_name'], $id);
			$this->view->message('success', 'Post added');
		}
		$this->view->render('Add text');
	}
	...
	public function editAction()
	{
		...
		if(!empty($_POST))
		{
			...
			{
				$this->model->postUploadImpage($_FILES['img']['tmp_name'], $this->route['id']);
			}
	...
	{
		if(!$this->model->isPostsExists($this->route['id']));
		{
			$this->view->errorCode(404);
		}
